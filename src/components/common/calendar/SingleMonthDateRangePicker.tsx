...
export default function SingleMonthDateRangePicker({
  value,
  onChange,
  minRangeDays = 0,
  ...
  const getDateClass = (date: Date) => {
    if (date < today)
      return "text-gray-400 line-through cursor-not-allowed bg-transparent";
          ...
          {dates.map((date, index) =>
            date ? (
              <button
                ...
                disabled={date < today}
                className={`p-1 rounded text-center ${getDateClass(date)}`}
              >
                {date.getDate()}
              </button>
            ) : (
